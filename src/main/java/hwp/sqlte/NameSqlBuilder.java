 ...
 */
public class NameSqlBuilder implements Sql {
    private List<Object> args = new ArrayList<>();
    private StringBuilder sql = new StringBuilder();

    private String errmsg;

    public NameSqlBuilder() {
    ...
    public NameSqlBuilder args(Object... args) {
        if (this.args.size() > 0) {
            this.args.clear();
        }
    ...
    public NameSqlBuilder where(Consumer<Where> where) {
        Where w = new Where();
        where.accept(w);
        this.sql.append(w);
        return this;
    ...
    public NameSqlBuilder order(Where where) {
        return this;
    }

    public NameSqlBuilder limit(int offset, int size) {
        return this;
    }
...
//                long count = sql.chars().filter(c -> c == '?').count();
                ...
                builder.append(sql);
                Collections.addAll(NameSqlBuilder.this.args, args);
            }
            return this;
        }
    ...
    public static void main(String[] args) {
        NameSqlBuilder sql = new NameSqlBuilder();
